...
void run_scan(char const *fn_name, void fn(int const, int const*, int*),
		int const num_elem, int *a, int *r) {
	double t1 = get_wtime();
	fn(num_elem, a, r);
	double t2 = get_wtime();
	cout << fn_name << ": " << fixed << (t2 - t1) << endl;
...
void run_test_gpu(char const *fn_name, void fn(int const, int const*, int*),
		int const num_elem, int const *a, int *s, int *r) {
	std::copy(a, a + num_elem, r);
	...
	run_scan(fn_name, fn, num_elem, d_a, d_r);
	CHECK(cudaMemcpy(r, d_r, size, cudaMemcpyDeviceToHost));
	CHECK(cudaFree(d_r));
	CHECK(cudaFree(d_a));
	verify_result(fn_name, num_elem, s, r);
...
//	int const num_elem = 32768;
	...
	run_test_gpu("scan_fan_cuda128", scan_fan_cuda128, num_elem, a, s, r);
	run_test_gpu("scan_fan_cuda256", scan_fan_cuda256, num_elem, a, s, r);
	run_test_gpu("scan_fan_cuda512", scan_fan_cuda512, num_elem, a, s, r);
